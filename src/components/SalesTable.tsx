import type { SaleItem } from '../../shared/sales'
import { formatCurrency, formatDisplayDate, formatThousandNumber } from '@/utils/format'
        ...
        <table className="min-w-full border-collapse text-xs lg:text-sm">
          <thead className="bg-zinc-950 text-left text-[11px] uppercase tracking-[0.28em] text-amber-50">
            <tr>
              ...
              <th className="px-4 py-3 text-right lg:px-6 lg:py-4">Jumlah</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              Array.from({ length: 4 }).map((_, index) => (
                <tr key={index} className="border-b border-zinc-100">
                  <td className="px-4 py-3 lg:px-6 lg:py-4" colSpan={4}>
              ...
              items.map((item, index) => (
                <tr
                  key={item.id}
                  className={cn(
                    'border-b border-zinc-100 transition hover:bg-emerald-50/50',
                    index % 2 === 0 ? 'bg-white' : 'bg-zinc-50/50',
                  )}
                >
                  ...
                  <td className="px-4 py-3 text-right font-bold text-emerald-700 lg:px-6 lg:py-4">
              ...
              <tr>
                <td className="px-4 py-10 text-center text-zinc-500 lg:px-6 lg:py-12" colSpan={4}>
                  Belum ada penjualan.
                </td>
              </tr>
            )}
